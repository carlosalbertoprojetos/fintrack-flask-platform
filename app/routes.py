...
from app import db
from app.models import Expense, User, Category, Transaction
from app.forms import ExpenseForm, LoginForm, RegistrationForm, TransactionForm, CategoryForm
...
def dashboard():
    ...
    expense_chart_data = [{'name': cat, 'value': float(total)} for cat, total in top_expense_categories]
    income_chart_data = [{'name': cat, 'value': float(total)} for cat, total in top_income_categories]
...
def add_expense():
    form = ExpenseForm()
    if form.validate_on_submit():
        # Correto: usar apenas os campos que pertencem a Expense
        expense = Expense(
            name=form.name.data,
...
def edit_expense(id):
    ...
    if form.validate_on_submit():
        ...
        expense.exclusive = form.exclusive.data
        db.session.commit()
...
def delete_expense(id):
    ...
    if Transaction.query.filter_by(description_id=expense.id).first():
        flash('Não é possível excluir uma descrição que está sendo usada em transações.', 'danger')
    else:
...
def transactions():
    ...
    category_chart_data = [{'name': cat, 'value': val} for cat, val in category_data.items()]
...
def add_transaction():
    ...
    # Carregar descrições predefinidas (inicialmente vazias)
    form.description_id.choices = [(0, 'Selecione uma descrição predefinida')]
    ...
    if form.validate_on_submit():
        ...
        if form.description_id.data and form.description_id.data > 0:
            transaction.description_id = form.description_id.data
    ...
    return render_template('add_transaction.html', form=form, edit=False)
...
def edit_transaction(id):
    ...
    if transaction.category_id:
        ...
        form.description_id.choices.extend([(e.id, e.name) for e in expenses])
    ...
    if form.validate_on_submit():
        ...
        if form.description_id.data and form.description_id.data > 0:
            ...
            transaction.description_id = None
...
def add_category():
    form = CategoryForm()
    if form.validate_on_submit():
        # Correto: usar apenas os campos que pertencem a Category
        category = Category(
            name=form.name.data,
            type=form.type.data,
            icon=form.icon.data,
...
def edit_category(id):
    ...
    if form.validate_on_submit():
        ...
        category.exclusive = form.exclusive.data
        db.session.commit()
...
def delete_category(id):
    ...
    if Transaction.query.filter_by(category_id=category.id).first():
        flash('Não é possível excluir uma categoria que está sendo usada em transações.', 'danger')
    else:
...
def reports():
    ...
    if report_type == 'monthly':
        ...
        income_chart_data = [{'name': cat, 'value': float(total)} for cat, total in income_by_category]
        expense_chart_data = [{'name': cat, 'value': float(total)} for cat, total in expense_by_category]
        ...
        annual_income = sum(item['income'] for item in monthly_data)
        annual_expense = sum(item['expense'] for item in monthly_data)
        ...
        # Preparar dados para gráficos
        top_expense_chart_data = [{'name': cat, 'value': float(total)} for cat, total in top_expense_categories]
        top_income_chart_data = [{'name': cat, 'value': float(total)} for cat, total in top_income_categories]
